      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-1.5">
            <div className="p-0.5 rounded" style={{ background: "#FFF0F0", borderRadius: "3px" }}>
              <Sparkles className="w-3 h-3" style={{ color: "#D4372B" }} />
            </div>
            <div>
              <h2 className="text-xs lg:text-sm font-semibold" style={{ color: "#0A0A0A", fontFamily: amazonFont }}>
                Mode pour toute la famille
              </h2>
              <p className="text-[9px] lg:text-[10px]" style={{ color: "#AAAAAA", fontFamily: amazonFont }}>
          ...
          <Link
            ...
            className="text-[9px] lg:text-[10px] inline-flex items-center gap-0.5 transition-all duration-200 hover:gap-1 hover:opacity-70"
            style={{ color: "#AAAAAA", fontFamily: amazonFont }}
          >
            Voir toute la mode
            <ChevronRight className="w-2 h-2 lg:w-2.5 lg:h-2.5" />
          </Link>
        </div>

        {/* Grille catégories avec animation de fond comme deal-countdown */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
          {categories.map((category, index) => {
            ...
            return (
              <div 
                key={category.id} 
                className="p-1.5 space-y-1.5 transition-all duration-1000 ease-in-out"
                style={{ 
              ...
              >
                ...
                <Link href={category.href} className="group block">
                  <div
                    ...
                    style={{ border: "0.5px solid #ECECEC", borderRadius: "4px" }}
                  >
                    ...
                    <div className="absolute inset-0 flex flex-col justify-end p-2">
                      <div className="flex items-center gap-1 mb-0.5">
                        <div className="bg-white/90 backdrop-blur-sm p-0.5 rounded" style={{ border: "0.5px solid rgba(0,0,0,0.08)" }}>
                          <Icon className="w-2 h-2" style={{ color: "#0A0A0A" }} />
                        </div>
                        <h3 className="text-[10px] font-semibold text-white" style={{ fontFamily: amazonFont }}>
                          {category.name}
                        </h3>
                      </div>

                      <div className="flex items-center justify-between">
                        <span className="text-[8px] text-white/80" style={{ fontFamily: amazonFont }}>
                          {category.productCount} produits
                        </span>
                        <span className="inline-flex items-center gap-0.5 text-[9px] font-medium text-white transition-all duration-200 group-hover:gap-1" style={{ fontFamily: amazonFont }}>
                          Explorer
                    ...
                    {index === 1 && (
                      <div
                        className="absolute top-1.5 right-1.5 text-[7px] font-medium px-1 py-0.5 rounded-full"
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  {category.products.slice(0, 2).map((product) => (
                    ...
                    >
                      <div
                        className="bg-white p-1 transition-all duration-300 hover:shadow-sm"
                        style={{ border: "0.5px solid #ECECEC", borderRadius: "4px" }}
                      >
                        <div className="relative aspect-square mb-0.5 overflow-hidden" style={{ background: "#FAFAFA", borderRadius: "3px" }}>
                          <Image
                            src={product.image}
                            alt={product.name}
                            fill
                        ...
                        <h4 className="text-[8px] font-medium line-clamp-1" style={{ color: "#0A0A0A", fontFamily: amazonFont }}>
                          {product.name}
                        </h4>
                        
                        <p className="text-[9px] font-semibold mt-0.5" style={{ color: "#D4372B", fontFamily: amazonFont }}>
